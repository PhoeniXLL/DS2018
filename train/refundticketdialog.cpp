...
#include "ui_refundticketdialog.h"
#include <QProcess>
#include <QMessageBox>
#include <QDebug>

refundTicketDialog::refundTicketDialog(QWidget *parent, int _id, QString data) :
    QDialog(parent),
    ui(new Ui::refundTicketDialog),
    id(_id)
{
    ui->setupUi(this);
    if (id) {
...
refundTicketDialog::~refundTicketDialog()
{
    delete ui;
}

void refundTicketDialog::on_confirmButton_clicked()
{
    ...
    if (num == "") {
        QMessageBox::critical(this, trUtf8("错误"), trUtf8("退票数量不应为空"));
        return;
    }
    if (train_id == "") {
    ...
    QString command = "refund_ticket " + id + " " + num + " " + train_id + " " + loc1 + " " + loc2 + " " + date + " " + ticket_kind + "\n";
    qDebug() << command;
    ...
    qDebug() << ret;
    if (ret) {
        QMessageBox::information(this, trUtf8("成功"), trUtf8("退票成功"));
        close();
    } else {
        QMessageBox::critical(this, trUtf8("错误"), trUtf8("退票失败"));
        if (!this->id) {
            ui->lineEdit_1->clear();
        }
